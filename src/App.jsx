...
function AdminPedidos({ db, setDb, adminRole }) {
  ...
  const reenviarACadete = (order) => {
    const customer = order.customer || order.customerInfo || {};
    let text = `*NUEVO DELIVERY*\n\n`;
    text += `*Cliente:* ${customer.name}\n`;
    text += `*Tel:* ${customer.phone}\n`;
    text += `*Dirección:* ${customer.address}\n`;
    if (customer.coords) {
      text += `*Ubicación:* https://maps.google.com/?q=${customer.coords.lat},${customer.coords.lng}\n`;
    }
    text += `*A cobrar:* ${formatCurrency(order.total)}\n`;
    text += `*Pago:* ${order.paymentDetails}\n`;
    if (customer.notes) text += `*Aclaraciones:* ${customer.notes}\n`;
