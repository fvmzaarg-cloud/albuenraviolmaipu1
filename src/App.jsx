...
function PrintTicket({ order, isComanda, onClose, db }) {
  ...
  const isAnulado = order.status === "Cancelado" || order.status === "Anulado";
...
function AdminCajaVentas({
  ...
  return (
    <div className="flex flex-col h-full relative animate-fadeIn bg-gray-100">
      ...
      {/* MODIFICACIÓN: Aumentamos el padding-bottom (pb-[150px]) para que el último producto 
          se pueda ver bien por encima de la ventana flotante del carrito. */}
      <div className="flex-1 overflow-y-auto p-3 pb-[150px] hide-scrollbar">
      ...
      {/* --- PANEL INFERIOR FLOTANTE (ESTA ES LA SOLUCIÓN) --- */}
      <div className="fixed bottom-[80px] left-4 right-4 bg-white shadow-[0_5px_30px_rgba(0,0,0,0.2)] flex flex-col z-[60] rounded-2xl transition-all duration-300 border border-gray-300">
        <button
          onClick={() => {
          ...
          className={`w-full flex justify-between items-center p-4 transition-colors ${
            isCartExpanded
              ? "bg-gray-100 border-b border-gray-200 text-gray-800 rounded-t-2xl"
              : "bg-[#c82a2a] text-white rounded-2xl active:bg-red-800"
          }`}
        >
        ...
        {isCartExpanded && (
          <div className="flex flex-col bg-white max-h-[65vh] rounded-b-2xl overflow-hidden">
            {!showPaymentOptions && (
...
function AdminCajaRecibos({
    ...
    setConfirmObj({
      ...
      action: () => {
        setDb((prev) => {
          ...
          const isAnulado =
            order.status === "Cancelado" || order.status === "Anulado";
          if (!isAnulado) {
  ...
  const borrarTodosLosPedidos = () => {
    setConfirmObj({
      msg: "⚠️ ATENCIÓN: ¿Estás totalmente seguro de borrar TODOS los recibos? Se restaurará el stock de los no anulados.",
      action: () => {
        setDb((prev) => {
          let updatedProducts = [...prev.products];
  ...
  return (
    <div className="space-y-4 animate-fadeIn pb-24">
      ...
      <div className="flex justify-between items-center">
        ...
        {/* BOTÓN GLOBAL DE BORRAR HISTORIAL */}
        {adminRole === "propietario" && receipts.length > 0 && (
          <button
      ...
      {receipts.length === 0 ? (
        ...
        <div className="space-y-3">
          {receipts.map((order) => {
            // Validamos ambas palabras por las dudas
            const isAnulado =
              order.status === "Cancelado" || order.status === "Anulado";
            ...
            return (
              <div
                ...
                className={`border rounded-xl p-3 shadow-sm transition-all ${
                  isAnulado
                    ? "bg-gray-100 border-gray-300 opacity-75 grayscale-[20%]" // Gris claro para anulados
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <span className="text-[10px] font-mono bg-gray-200/60 px-2 py-0.5 rounded text-gray-700">
                      #{order.id}
                    </span>
                    <p
                  ...
                  <div className="text-right">
                    <p
                      className={`font-black ${
                        isAnulado ? "text-gray-400" : "text-blue-700"
                      }`}
                    >
                      {formatCurrency(order.total || 0)}
                    </p>

                    {/* ETIQUETA DE ESTADO COLOR ROJO */}
                    <span
                ...
                <div className="flex justify-between gap-2 mt-2 pt-2 border-t border-gray-200/60">
                  {order.status === "Pendiente" && (
                    <button
                      onClick={() => {
                        setCart(order.items || []);
    ...
    <div className="flex flex-col h-full bg-gray-50 relative">
      ...
      {newOrderPopup && (
        <div className="fixed inset-0 bg-black/70 z-[9999] flex items-center justify-center p-4 animate-fadeIn">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl border-4 border-red-500 text-center animate-bounce">
            ...
            <button
              ...
              className="w-full bg-[#c82a2a] text-white font-bold py-4 rounded-xl hover:bg-red-800 text-lg shadow-lg"
            >
              Ver Pedido
            </button>
          </div>
...
function AdminPedidos({ db, setDb, adminRole }) {
  ...
  const deleteOrder = (orderId) => {
    setConfirmObj({
      msg: "¿Seguro que querés eliminar este pedido permanentemente? Ya no aparecerá en el sistema.",
      action: () => {
        setDb((prev) => {
          const order = prev.orders.find((o) => o.id === orderId);
          if (!order) return prev;
  ...
  const borrarTodosLosPedidos = () => {
    setConfirmObj({
      msg: "⚠️ ATENCIÓN FRANCO: ¿Estás totalmente seguro de borrar TODOS los pedidos? Se restaurará el stock de los no anulados.",
      action: () => {
        setDb((prev) => {
          let updatedProducts = [...prev.products];
          prev.orders.forEach((order) => {
  ...
  const reenviarACadete = (order) => {
    ...
    if (customer.coords) {
      text += `*Ubicación GPS:* https://maps.google.com/?q=${customer.coords.lat},${customer.coords.lng}\n`;
    }
    if (customer.notes) {
      text += `*Aclaraciones:* ${customer.notes}\n`;
    }

    text += `\n*Detalle del pedido:*\n`;
    items.forEach((item) => {
      const cantidad = Number(item.quantity)
        .toFixed(item.product?.unitType === "peso" ? 3 : 0)
        .replace(/\.000$/, "");
      const unidad = item.product?.unitType === "peso" ? "kg" : "u";
      text += `· ${cantidad} ${unidad} x ${item.product?.name}\n`;
    });

    text += `\n*Subtotal:* ${formatCurrency(subtotal)}\n`;
    text += `*Envío:* ${formatCurrency(order.shippingCost || 0)}\n`;
    text += `*TOTAL A COBRAR:* ${formatCurrency(order.total)}\n`;
    text += `*Pago:* ${order.paymentDetails}\n`;

    window.open(`https://wa.me/?text=${encodeURIComponent(text)}`, "_blank");
  };

  const statusColors = {
    Recibido: "bg-[#fef08a] text-yellow-900 border border-yellow-300",
    Pendiente: "bg-orange-100 text-orange-800 border border-orange-300",
    Entregado: "bg-[#dcfce7] text-green-800 border border-green-300",
    Rendido: "bg-purple-100 text-purple-800 border border-purple-300",
  };
              ...
              >
                {isAnulado && (
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10 overflow-hidden">
                    <div className="text-red-500/10 font-black text-6xl md:text-7xl uppercase tracking-widest transform -rotate-12 border-8 border-red-500/10 rounded-3xl px-8 py-4 select-none">
                      ANULADO
                    </div>
                  </div>
                )}
                ...
                <div
                  className={`relative z-20 text-sm border p-3 rounded-xl mb-3 space-y-1 ${
                    isAnulado
                      ? "bg-gray-100 border-gray-200 text-gray-500"
                      : "bg-[#f8fafc] border-gray-100 text-gray-800"
                  }`}
                >
                  <p>
                  ...
                  <p>
                    <strong>Tipo:</strong>{" "}
                    {order.type?.toUpperCase() || "LOCAL"}
                  </p>
                  <p
                    className={`font-bold flex items-center gap-1.5 mt-1 ${
                      isAnulado ? "text-gray-500" : "text-blue-700"
                    }`}
                  >
                    <CreditCard
                      size={16}
                      className={isAnulado ? "text-gray-400" : "text-blue-500"}
                    />{" "}
                    Pago: {order.paymentDetails || "Efectivo"}
                  </p>
                  {order.type === "delivery" && (
                    <p className="mt-1">
                  ...
                  {customer.notes && (
                    <p
                      className={`font-bold mt-1 p-1.5 rounded border ${
                        isAnulado
                ...
                <div
                  className={`relative z-20 text-sm space-y-1.5 mb-3 px-1 ${
                    isAnulado ? "text-gray-500" : "text-gray-700"
                  }`}
                >
                  {items.map((i, idx) => (
                    <div key={idx} className="flex justify-between">
                      <span>
                        {Number(i.quantity)
                          .toFixed(i.product?.unitType === "peso" ? 3 : 0)
                          .replace(/\.000$/, "")}{" "}
                        {i.product?.unitType === "peso" ? "kg" : "u"} x{" "}
                        {i.product?.name}
                      </span>
                ...
                <div className="relative z-20 border-t border-gray-200 pt-3 space-y-1 mb-4 px-1">
                  ...
                  <div
                    className={`flex justify-between items-center font-black text-xl pt-2 ${
                      isAnulado ? "text-gray-500" : "text-gray-900"
                    }`}
                  >
                    <span>Total:</span>
                    <span>{formatCurrency(order.total || 0)}</span>
                ...
                {order.type === "delivery" && customer.coords && !isAnulado && (
                  <div className="relative z-20 bg-[#eff6ff] border border-blue-200 rounded-xl p-3 mb-4">
                    <p className="font-bold text-blue-900 flex items-center gap-1 mb-1 text-sm">
                      <MapPin size={16} className="text-red-500" /> Ubicación
                      GPS:
                    </p>
                    <a
                ...
                <div className="relative z-20">
                  {order.type === "delivery" && !isAnulado ? (
                    <button
                      onClick={() => reenviarACadete(order)}
                      className="w-full bg-[#22c55e] text-white p-3.5 rounded-xl font-bold text-base flex items-center justify-center gap-2 shadow-md hover:bg-green-600 transition-colors active:scale-95"
                    >
                    ...
                    <button
                      ...
                      className="w-full bg-blue-50 text-blue-700 p-3.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 border border-blue-200 hover:bg-blue-100 transition-colors active:scale-95"
                    >
                      <Calculator size={18} /> Enviar a Caja
                    </button>
                  ) : null}
                </div>
              </div>
